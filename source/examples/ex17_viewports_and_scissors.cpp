...
class ViewportsApplication : public our::Application
{
    ...
    void onInitialize() override
    {
        ...
        areas.push_back({glm::ivec2(0, 0), half_size, glm::ivec2(0, 0), half_size, true, glm::vec4(0.1f, 0.2f, 0.3f, 1.0f), true, default_camera, {}});
        areas.push_back({half_size, half_size, half_size, half_size, true, glm::vec4(0.3f, 0.2f, 0.1f, 1.0f), true, default_camera, {}});
        for (auto &area : areas)
        {
    ...
    void onImmediateGui(ImGuiIO &io) override
    {
        ...
        our::ReorderableList(
            ...
            [this](size_t index)
            {
                areas.insert(areas.begin() + index, {{0, 0}, {100, 100}, {0, 0}, {100, 100}, true, {1, 1, 1, 1}, true, this->default_camera, {}});
                areas[index].controller.initialize(this, &(areas[index].camera));
            },
            [this](size_t index)
            {
                areas.erase(areas.begin() + index);
                if (areas.size() <= this->selected_camera)
